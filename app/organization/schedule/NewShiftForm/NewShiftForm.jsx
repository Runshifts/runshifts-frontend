...
export default function NewShiftForm({ onCancel, newShiftDetails, handleNewShift }) {
  ...
  const shiftDurationDate = useMemo(() => newShiftDetails?.shiftDate, [newShiftDetails?.shiftDate])
  const handleScheduleSelection = useCallback((selected) => {
    const shiftManagement = shiftManagements.find(it => it._id === selected || it.name.toLowerCase() === selected.toLowerCase())
    if (!shiftManagement || !shiftDurationDate) return null
    const startTime = new Date(shiftManagement.startTime)
    startTime.setFullYear(shiftDurationDate.getFullYear(), shiftDurationDate.getMonth(), shiftDurationDate.getDate())
    const endTime = new Date(shiftManagement.endTime)
    endTime.setFullYear(shiftDurationDate.getFullYear(), shiftDurationDate.getMonth(), shiftDurationDate.getDate())
    setShiftData(prev => ({ ...prev, schedule: shiftManagement._id, startTime, endTime }))
  ...
  const handleSubmit = useCallback(async (e) => {
    e.preventDefault()
    if (!shiftData.location) return toast.error("Please select a location for this shift")
    if (!shiftData.startTime || !shiftData.endTime) return toast.error("Please select a schedule for this shift")
    setLoading(true)
  ...
  return (
    <section className="w-[95dvw] py-[24px] px-[24px] md:px-[40px] max-w-[576px] bg-white rounded-[16px]">
      ...
      <form className="flex gap-y-[16px] flex-col items-start w-full" onSubmit={handleSubmit}>
        <div className="grid grid-cols-2 w-full gap-[16px]">
          ...
          <FormInputAndLabel
            ...
            inputProps={{
              ...
              value:
                `${newShiftDetails.assignee?.firstName || ""} ${newShiftDetails.assignee?.lastName || ""
                  }`.trim() ||
                newShiftDetails.assignee?.email ||
                "",
